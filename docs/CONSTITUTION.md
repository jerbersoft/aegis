...
- Workflow-document branch policy: `.work/` Markdown workflow artifacts (`feature.md`, `TASK.md`, `developer_handoff.md`, `implementation_summary.md`, `testing_results.md`, `review_results.md`, `ACCEPTANCE.md`, and `FEATURE_SUMMARY.md`) are canonical only in the main workspace on the owner-selected base branch. Agents must not create, copy, or update those `.work/` Markdown artifacts inside implementation worktree branches.
- `Orchestrator` may commit and push only on the recorded implementation worktree branch when feature publication is part of an explicitly user-approved workflow. For this repository, an owner command such as `accept this feature` counts as that explicit publication approval and authorizes `Orchestrator` to commit eligible unpublished feature changes, push the recorded worktree branch, and open the resulting PR against the recorded base branch. `Orchestrator` must not commit directly on a base branch, push directly to a base branch, force-push, or merge the PR.
