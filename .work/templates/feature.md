...
- After `ACCEPTANCE.md` is created, `orchestrator` should verify `runtime` is available, ask it to proactively prepare the owner acceptance environment from the recorded worktree, record the resulting environment/process state here, and present an owner-facing preview of the acceptance guide.
