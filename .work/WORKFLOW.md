...
- This preparation must follow the acceptance guide itself: if `ACCEPTANCE.md` requires `Aegis.AppHost` or other local runtime processes, `runtime` should first install any required worktree-local dependencies with the project's lockfile-backed install command, then start the processes from the recorded worktree, verify the expected owner entry path is reachable, and leave the environment ready for owner testing unless the acceptance guide explicitly says not to keep it running.
- `orchestrator` should treat missing worktree-local dependencies as an environment-preparation concern, not as evidence that the environment is ready.
