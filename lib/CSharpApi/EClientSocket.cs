...
namespace IBApi
{
    ...
    public class EClientSocket : EClient, EClientMsgSink
    {
        ...
        void EClientMsgSink.serverVersion(int version, string time)
        {
            ...
            {
                wrapper.error(clientId, Util.CurrentTimeMillis(), EClientErrors.UNSUPPORTED_VERSION.Code,
                    EClientErrors.UNSUPPORTED_VERSION.Message, "");
                return;
            }
        ...
        public void eConnect(string host, int port, int clientId, bool extraAuth)
        {
            ...
            if (isConnected)
            {
                wrapper.error(IncomingMessage.NotValid, Util.CurrentTimeMillis(), EClientErrors.AlreadyConnected.Code,
        ...
        */
        public void redirect(string host)
        {
            if (!allowRedirect)
            {
                wrapper.error(clientId, Util.CurrentTimeMillis(), EClientErrors.CONNECT_FAIL.Code,
                    EClientErrors.CONNECT_FAIL.Message, "");
                return;
            }
...
//             
            ...
            {
                SetWindowsKeepalive(tcpClient.Client, keepAliveTime, keepAliveInterval);
            }
            else if (RuntimeInformation.IsOSPlatform(OSPlatform.Linux))
            {
                SetLinuxKeepalive(tcpClient.Client, keepAliveTime, keepAliveInterval, 5);
            }
            else if (RuntimeInformation.IsOSPlatform(OSPlatform.OSX))
            {
                SetMacOSKeepalive(tcpClient.Client, keepAliveTime, keepAliveInterval, 5);
            }
            else
            {
                Console.WriteLine("Warning: Advanced keepalive settings not supported on this platform");
            }
        }
        ...
        private void SetWindowsKeepalive(Socket socket, uint keepAliveTime, uint keepAliveInterval)
        {
            // Windows uses IOControl with SIO_KEEPALIVE_VALS
            ...
            BitConverter.GetBytes(keepAliveTime * 1000).CopyTo(keepAlive, 4); // milliseconds
            BitConverter.GetBytes(keepAliveInterval * 1000).CopyTo(keepAlive, 8); // milliseconds

            socket.IOControl(IOControlCode.KeepAliveValues, keepAlive, null);
        }

        private void SetLinuxKeepalive(Socket socket, uint keepAliveTime, uint keepAliveInterval, int keepAliveRetryCount)
        {
            const int TCP_KEEPIDLE = 4; // Time before sending keepalive probes
            const int TCP_KEEPINTVL = 5; // Interval between keepalive probes
            const int TCP_KEEPCNT = 6; // Number of keepalive probes

            try
            {
                socket.SetRawSocketOption((int)SocketOptionLevel.Tcp, TCP_KEEPIDLE,
                    BitConverter.GetBytes(keepAliveTime));
                socket.SetRawSocketOption((int)SocketOptionLevel.Tcp, TCP_KEEPINTVL,
                    BitConverter.GetBytes(keepAliveInterval));
                socket.SetRawSocketOption((int)SocketOptionLevel.Tcp, TCP_KEEPCNT,
                    BitConverter.GetBytes(keepAliveRetryCount));
            }
            catch (SocketException ex)
            {
                Console.WriteLine($"Linux keepalive configuration failed: {ex.Message}");
            }
        }

        private void SetMacOSKeepalive(Socket socket, uint keepAliveTime, uint keepAliveInterval, int keepAliveRetryCount)
        {
            const int TCP_KEEPALIVE = 0x10; // macOS uses different constant than Linux
            const int TCP_KEEPINTVL = 0x101;
            const int TCP_KEEPCNT = 0x102;

            try
            {
                socket.SetRawSocketOption((int)SocketOptionLevel.Tcp, TCP_KEEPALIVE,
                    BitConverter.GetBytes(keepAliveTime));
                socket.SetRawSocketOption((int)SocketOptionLevel.Tcp, TCP_KEEPINTVL,
                    BitConverter.GetBytes(keepAliveInterval));
                socket.SetRawSocketOption((int)SocketOptionLevel.Tcp, TCP_KEEPCNT,
            ...
            {
                Console.WriteLine($"macOS keepalive configuration failed: {ex.Message}");
            }
        }
    }
}
